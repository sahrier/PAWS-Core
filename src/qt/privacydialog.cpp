                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zPAWS ought to be enough for anybody." - Bill Gates, 2017
    ui->zPAWSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzPAWSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzPAWS->setEnabled(false);
        ui->pushButtonMintzPAWS->setToolTip(tr("zPAWS is currently disabled due to maintenance."));

        ui->pushButtonSpendzPAWS->setEnabled(false);
        ui->pushButtonSpendzPAWS->setToolTip(tr("zPAWS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zPAWSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzPAWS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPAWS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zPAWS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zPAWS in ") +
...
void PrivacyDialog::on_pushButtonSpendzPAWS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPAWS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zPAWS
        sendzPAWS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zPAWS
    sendzPAWS();
}

void PrivacyDialog::on_pushButtonZPAWSControl_clicked()
{
    ZPAWSControlDialog* zPAWSControl = new ZPAWSControlDialog(this);
    zPAWSControl->setModel(walletModel);
    zPAWSControl->exec();
}

void PrivacyDialog::setZPAWSControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzPAWSSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzPAWS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid PAWS Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zPAWSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zPAWS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PAWS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zPAWS</b>";
    ...
    // use mints from zPAWS selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZPAWSControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZPAWSControlDialog::GetSelectedMints();
    }
    ...
    // Clear zpaws selector in case it was used
    ZPAWSControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zPAWS Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " PAWS, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zPAWS Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPAWS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zPAWS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zPAWS "));
    ui->labelzPAWSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    // Display AutoMint status

    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zPAWS </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zPAWS </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzPAWSSyncStatus->setVisible(fShow);
}
