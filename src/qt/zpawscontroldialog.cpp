...
#include "ui_zpawscontroldialog.h"
...
std::list<std::string> ZPAWSControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZPAWSControlDialog::listMints;

ZPAWSControlDialog::ZPAWSControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZPAWSControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZPAWSControlDialog::~ZPAWSControlDialog()
{
    delete ui;
}

void ZPAWSControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPAWSControlDialog::updateList()
{
...
void ZPAWSControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPAWSControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZPAWS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPAWSControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZPAWSControlDialog::GetSelectedMints()
{
...
void ZPAWSControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
